...
// GPTrack
static CString *
  createString(int size, char *beginStr)
{
...
void GPTrack::Create()
{
  ...
  UnderLayBitmapHeight.SetValue(
    (int)PROFILE_SPECIAL(UnderLayBitmapHeight, 300));
...
void GPTrack::NewTrack()
{
  ...
  for (int i = 0; i < size; i++) {
    TrackSection *t =
      new TrackSection(TrackSections, this, i, 10, 0, 0, 20, 20, 0, 0, NULL, 0, 1400, 1400, NULL, t_TrackSection);
    TrackSections->addElement(t);
  }
  for (int i = 0; i < 7; i++) {
    TrackSection *t =
      new TrackSection(PitlaneSections, this, i, 10, 0, 0, 20, 20, 0, 0, NULL, 0, 1400, 1400, NULL, t_PitSection);
  ...
  TrackCmd *Cmd1 =
    new TrackCmd(this, begin, 0xC9, 0, "LIGHT", 0, 0, 0, 0, 0, 0, 0, 0, 0);
  TrackCmd *Cmd2 =
    new TrackCmd(this, begin, 0xC9, 0, "LIGHT", 0, 0, 0, 0, 0, 0, 0, 0, 0);
  TrackCmd *Cmd3 =
    new TrackCmd(this, begin, 0xC9, 0, "LIGHT", 0, 0, 0, 0, 0, 0, 0, 0, 0);
  ...
  TrackCmd *Cmd5 =
    new TrackCmd(this, begin, 0xC8, 0, "LIGHT", 0, 0, 0, 0, 0, 0, 0, 0, 0);
  TrackCmd *Cmd6 =
    new TrackCmd(this, begin, 0xC8, 0, "LIGHT", 0, 0, 0, 0, 0, 0, 0, 0, 0);
  TrackCmd *Cmd7 =
    new TrackCmd(this, begin, 0xC8, 0, "LIGHT", 0, 0, 0, 0, 0, 0, 0, 0, 0);
  ...
  TrackCmd *kerbA =
    new TrackCmd(this, begin, KERB_DEF_1, 0, "KERB_DEF_1", 0, 1, 250, 1, 15);
  TrackCmd *kerbB =
    new TrackCmd(this, begin, KERB_DEF_2, 0, "KERB_DEF_2", 0, 1, 300, 1, 10);
  begin->addCommand(Light);
  begin->addCommand(kerbA);
  begin->addCommand(kerbB);

  TrackSection *pitroaden =
    (TrackSection *)TrackSections->elementAt(TrackSections->size() - 2);
...
{
  if (!circuitName.Compare("") || !circuitName.Compare("Unnamed Circuit"))
    return FALSE;
  return TRUE;
}
BOOL GPTrack::isCountryDefined()
{
  if (!countryName.Compare("") || !countryName.Compare("Unknown Country"))
    return FALSE;
  return TRUE;
}
...
void GPTrack::WriteInfo()
{
  ...
  wsprintf(buffer,
    "#GP2INFO|Name|%s|Country|%s|Created|Created by Track Editor "
    "written by Paul Hoad see (License.txt about distributing this "
    "track)|Author|%s|Year|%s|Event|%s|Desc|%s|Laps|%d|Slot|%d|Tyre|%d|"
    "LengthMeters|%d|LapRecord|%s|LapRecordQualify|%s|Split1|0x%x|"
    "Split2|0x%x|#",
    circuitName,
    countryName,
    authorName,
...
void GPTrack::ReadInfo(BOOLEAN complainNoGPInfo)
{
  ...
  if (trackdata[0] == '#' && trackdata[1] == 'G' && trackdata[2] == 'P') {
    ...
    // AfxMessageBox("DON'T PANIC but....\n\nGP2INFO Missing for this file!
    // (Your file has no GP2INFO attached) Track Editor will ask you for this
    // info before saving it to disk",MB_OK);
  ...
  while (count < last) {
    ...
    if (!strcmp(cat, "Name")) setCircuitName(val);
    if (!strcmp(cat, "Country")) setCircuitCountry(val);
    if (!strcmp(cat, "Author")) setCircuitAuthor(val);
    if (!strcmp(cat, "Year")) setCircuitYears(val);
    if (!strcmp(cat, "Event")) setCircuitEvent(val);
    if (!strcmp(cat, "Desc")) setCircuitDesc(val);
    if (!strcmp(cat, "Created")) setCircuitCopyright(val);
    if (!strcmp(cat, "Laps")) setCircuitLaps(atoi(val));
    if (!strcmp(cat, "Slot")) setBestSlotInfo(atoi(val));
    if (!strcmp(cat, "Tyre")) setTyreConsumption(atoi(val));
    if (!strcmp(cat, "LapRecord")) setLapRecord(val);
    if (!strcmp(cat, "LapRecordQualify")) setLapRecordQualify(val);
...
void GPTrack::ReadTrackFile(CDocument *pDoc, const char *filename)
{
  ...
  if (!fp) AfxMessageBox("Failed to open TrackFile for reading", MB_OK);
...
{
  WritePCByte(offset, (int)CCLineStartX.GetValue());
  offset += 1;
  WritePCByte(offset, CCLineStartXHigh.GetValue());
  offset += 1;

  // WritePCWord(offset,(int)CCLineStartYData.GetValue());offset+=2;
  WritePCWord(offset, (int)CCLineStartY.GetValue());
  offset += 2;

  WritePCWord(offset, CCUnknown1.GetValue());
  offset += 2;
  WritePCWord(offset, CCUnknown2.GetValue());
  offset += 2;
  WritePCWord(offset, CCUnknown3.GetValue());
  offset += 2;
  WritePCWord(offset, CCUnknown4.GetValue());
  offset += 2;
...
void GPTrack::RecreateData()
{
  if (isF1GP()) {
    ...
    if (CCLineToSetupGap > 4) {
      ...
      WritePCByte(offsetA, 'P');
      offsetA++;
      WritePCByte(offsetA, 'D');
      offsetA++;
      WritePCByte(offsetA, 'H');
      offsetA++;
    }
...
extern CPoint
  GetTrackPoint(double currrad, double angle, double length);
...
char *
  GPTrack::ReadNullTerminatedString(int start)
{
  char *buffer;
  char jamchar = (char)1;
...
void GPTrack::ReadOffsets(CDocument *pDoc)
{
  ...
  if (isF1GP()) {
    ...
    for (int i = 0; i < NumberOfObjects; i++) {
      ...
      InternalObject *obj = new InternalObject(
        this, pDoc, ObjectOffset + 0x1020, nextOffset + 0x1020);
      obj->ReadObject(this);
...
{
  int offsetstart = 0x1000;
  WritePCDWord(offsetstart, LargeNumber1.GetValue());
  offsetstart += 4;
  WritePCDWord(offsetstart, LargeNumber2.GetValue());
  offsetstart += 4;

  if (isF1GP()) {
    WritePCWord(offsetstart, CheckSumOffset - 0x1010);
    offsetstart += 2;// second pass
    WritePCWord(offsetstart, ListObjectOffset - 0x1010);
    offsetstart += 2;// second pass
    WritePCWord(offsetstart, TrackDataOffset - 0x1010);
    offsetstart += 2;// second pass
    WritePCWord(offsetstart, NumberOfObjects);
    offsetstart += 2;
  } else {
    WritePCDWord(offsetstart, CheckSumOffset - 0x1020);
    offsetstart += 4;// second pass
    WritePCDWord(offsetstart, ListObjectOffset - 0x1020);
    offsetstart += 4;// second pass
    WritePCDWord(offsetstart, TrackDataOffset - 0x1020);
    offsetstart += 4;// second pass
    WritePCDWord(offsetstart, SetupOffset - 0x1020);
    offsetstart += 4;// second pass
    WritePCDWord(offsetstart, TrackPitLaneStart - 0x1020);
    offsetstart += 4;// second pass

    NumberOfObjects = InternalObjectOffsets->size();// first pass

    WritePCDWord(offsetstart, NumberOfObjects);
    offsetstart += 4;

    for (int i = 0; i < NumberOfObjects; i++) {
      InternalObject *obj =
        (InternalObject *)InternalObjectOffsets->elementAt(i);
...
{
  WritePCWord(offset, TrackStartAngle.GetValue());
  offset += 2;
  WritePCWord(offset, TrackStartClimb.GetValue());
  offset += 2;
  WritePCWord(offset, TrackStartY.GetValue());
  offset += 2;
  WritePCWord(offset, TrackStartZ.GetValue());
  offset += 2;
  WritePCWord(offset, TrackStartX.GetValue());
  offset += 2;
  WritePCWord(offset, TrackBeginWidth.GetValue());
  offset += 2;

  WritePCWord(offset, TrackUnknown1.GetValue());
  offset += 2;// unknown
  WritePCWord(offset, C5Code.GetValue());
  offset += 2;// c5code
  WritePCByte(offset, BeginTrackWidthRight.GetValue());
  offset += 1;
  WritePCByte(offset, BeginTrackWidthLeft.GetValue());
  offset += 1;

  WritePCWord(offset, UnknownNumber.GetValue());
  offset += 2;

  for (int j = 0; j < UnknownNumberCount; j++) {
    WritePCWord(offset, UnknownNumbers[j].GetValue());
    offset += 2;
  }
...
int GPTrack::ReadTrack(int offset)
{
  ...
  if (UnknownNumber.GetValue() == 3)
    UnknownNumberCount = 4;
  ...
  while (count) {
    ...
    case 0xa1:
      ...
      break;// entry from pit road
    case 0xa2:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 0);
      break;// entry from pit road
    case 0xa3:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 0);
      break;// exit from pit road
    case 0xa4:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 0);
      break;// exit from pit road
    case 0xa7:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 2);
      break;// unknown
    case 0xa8:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 0);
      break;// unknown
    case 0xa6:
      ...
      break;// unknown
    case 0xaa:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 3);
      break;
    case 0xab:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 2);
      break;
    case 0xac:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 4);
      break;
    case 0xaf:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 2);
      break;// triangle
    ...
    case 0xb4: {
      // offset =
      // GetNWordUnknownCommand(SectionCmds,TrackCmd,TrackCmdValue,offset,2);break;
      // // trackWidthChangeLeft
      TrackWidthLeft =
        GetTrackWidthLeft(SectionCmds, TrackCmdValue, offset, TRUE, 0xb4);
      offset += 4;
    } break;
    case 0xb5: {
      // offset =
      // GetNWordUnknownCommand(SectionCmds,TrackCmd,TrackCmdValue,offset,2);//
      // trackWidthChangeRight
      TrackWidthRight =
        GetTrackWidthLeft(SectionCmds, TrackCmdValue, offset, FALSE, 0xb5);
      offset += 4;
    } break;
    case 0xb0:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 1);
      break;// unknown
    case 0xb1:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 1);
      break;// unknown
    ...
    case 0xb7:
      ...
      break;// unknown
    case 0xb8:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 13);
      break;// trees
    case 0xba:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 2);
      break;// unknown
    ...
    case 0xbf:
      ...
      break;// unknown
    case 0xbc:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 7);
      break;// texture
    ...
    case 0xc6:
      ...
      break;// unknown
    case 0xc9:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 8);
      break;// lights
    case 0xc8:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 7);
      break;// lights
    ...
    case 0xd4:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 0);
      break;// show pitlane exit
    case 0xd5:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 1);
      break;// unknown
    case 0xd6:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 0);
      break;// show pitlane entry
    case 0xd7:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 0);
      break;// unknown
    case 0xd8:
    ...
    case 0xe0:
      ...
      break;// kirb height
    // case 0xe1:  offset =
    // GetNWordUnknownCommand(SectionCmds,TrackCmd,TrackCmdValue,offset,1);break;
    // // kirb height
    case 0xe1:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 0);
      break;// unknown
    ...
    case 0x8a:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 5);
      break;// track marking
    case 0x8b:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 5);
      break;// track marking
    ...
    case 0xff: {
      if (TrackCmdValue != 0xff) AfxMessageBox("Not true end of message");
    ...
    case 0x95: {
      // offset =
      // GetNWordUnknownCommand(SectionCmds,TrackCmd,TrackCmdValue,offset,1);//
      // unknown
    ...
    case 0x98:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 1);
      break;// changing fence
    case 0x99:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 1);
      break;// changing fence right
    case 0x9a:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 2);
      break;// changing fence left
    case 0x9b:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 0);
      break;// changing fence left
    ...
    case 0xA9:
      ...
      break;
      // case 0xDB:  offset =
      // GetNWordUnknownCommand(SectionCmds,TrackCmd,TrackCmdValue,offset,0);break;
...
{
  TrackCmd *trackcmd =
    new TrackCmd(this, NULL, cmd, offset - 2, "Unknown TrackCmd");
...
{
  TrackCmd *trackcmd =
    new TrackCmd(this, NULL, 0x85, offset - 2, "Track Width");

  int Position = getTrackDataWord(offset);
  offset += 2;
  int TrackWidth = getTrackDataWord(offset);
  offset += 2;
...
int GPTrack::GetTrackGrip(Vector *SectionCmds, int arg1, int offset, int cmd)
{
  ...
  int GripLevel = getTrackDataWord(offset);
  offset += 2;
  trackcmd->AddWordArgument(arg1, "Unknown");
...
{
  TrackCmd *trackcmd =
    new TrackCmd(this, NULL, cmd, offset - 2, "Track Width Left/Right");

  int Position = getTrackDataWord(offset);
  offset += 2;
  int newTrackWidth = getTrackDataWord(offset);
  offset += 2;
  trackcmd->AddWordArgument(arg1, "Unknown");
  trackcmd->AddWordArgument(Position, "Position");
...
TrackObject *
  GPTrack::GetTrackObject(int offset, int len, Vector *SectionCmds)
{
...
int GPTrack::GetTrackSection(int offset, int length, Vector *objects, double widthLeft, double widthRight, Vector *sectionCmds, int gripLevel)
{
  ...
  TrackSection *ts = new TrackSection(
    TrackSections, this, idx, length, angle, climb, wl, wr, RS, CB, objects, trackDataOffset, widthLeft, widthRight, sectionCmds, t_TrackSection);
...
int GPTrack::ReadPitLane(int offset)
{
  ...
  while (count > 0) {
    ...
    case 0xb4:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 2);
      break;// trackWidthChangeLeft
    case 0xb5:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 2);
      break;// trackWidthChangeRight
    case 0xb6:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 1);
      break;// unknown
    case 0xb7:
    ...
    case 0x88:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 1);
      break;// pitlane unknown line?
    case 0x89:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 1);
      break;// pitlane unknown line? needed
    case 0x8a:
      offset = GetNWordUnknownCommand(SectionCmds, TrackCmd, TrackCmdValue, offset, 5);
      break;// track marking
    case 0x8b:
...
int GPTrack::GetPitLaneSection(int offset, int length, Vector *objects, double widthLeft, double widthRight, Vector *sectionCmds, int gripLevel)
{
  ...
  TrackSection *ts = new TrackSection(
    PitlaneSections, this, idx, length, angle, climb, wl, wr, RS, CB, objects, trackDataOffset, widthLeft, widthRight, sectionCmds, t_PitSection);
  ts->setGripLevel(gripLevel);
...
Camera *
  GPTrack::GetCamera(int offset, int num)
{
...
{
  int Data1l = getTrackDataByte(offset);
  offset += 1;
  int Data1h = getTrackDataByte(offset);
  offset += 1;
  short Data2 = (short)getTrackDataWord(offset);
  offset += 2;
  CCUnknown1.SetValue(getTrackDataWord(offset));
  offset += 2;
  CCUnknown2.SetValue(getTrackDataWord(offset));
  offset += 2;
  CCUnknown3.SetValue(getTrackDataWord(offset));
  offset += 2;
  CCUnknown4.SetValue(getTrackDataWord(offset));
  offset += 2;
  ...
  while (count > 0) {
    ...
    case 0x50: {
      // unk
      int DataA = getTrackDataWord(offset);
      offset += 2;
      // direction
      int DataB1 = getTrackDataWord(offset);
      offset += 2;
      // radius
      int Radius = getTrackDataWord(offset);
      offset += 2;

      // int v =
      // (HIWORD(DataB1)*0x10000000)+(LOWORD(DataB1)*0x10000)+((HIWORD(Radius)*0x100))+(LOWORD(Radius));
      ...
      CCLineSection *ccline = new CCLineSection(
        this, TrackCmdValue, Radius, DataA, DataB, 0, 0, 0x50, trackoffset);
    ...
    case 0x70: {
      int DataA = getTrackDataWord(offset);
      offset += 2;
      short DataB = getTrackDataWord(offset);
      offset += 2;
      int Radius = getTrackDataWord(offset);
      offset += 2;
      ...
      int DataC = getTrackDataWord(offset);
      offset += 2;
      int DataD = getTrackDataWord(offset);
      offset += 2;
      CCLineSection *ccline =
        new CCLineSection(this, TrackCmdValue, Radius, DataA, DataB, DataC, DataD, 0x70, trackoffset);
      totalLength += TrackCmdValue;
      CCLineSections->addElement(ccline);
    } break;
    case 0x00: {
...
int GPTrack::PutCCSection(CCLineSection *t)
{
  ...
  WritePCByte(offset, TrackCmdValue);
  offset += 1;
  WritePCByte(offset, TrackCmd);
  offset += 1;

  switch (TrackCmd) {
  case 0x50: {
    WritePCWord(offset, (int)t->getArg1());
    offset += 2;
    WritePCWord(offset, (int)t->getArg2());
    offset += 2;
  ...
  case 0x70: {
    ...
    WritePCWord(offset, (int)t->getArg2());
    offset += 2;
    WritePCWord(offset, (int)t->getRadius());
    offset += 2;
    WritePCWord(offset, (int)t->getArg3());
    offset += 2;
    WritePCWord(offset, (int)t->getArg4());
    offset += 2;
  } break;
  }
  return 1;
...
TrackObjectDefinition *
  GPTrack::getObjectDefinition(int code)
{
  // check the divide by 4 location first
  int possibleLocation = code / 16;

  TrackObjectDefinition *tPossible =
    (TrackObjectDefinition *)TrackObjectDefintions->elementAt(
      possibleLocation);
...
static double
  DMStoAngle(int degrees, int minutes)
{
  ...
  for (int i = 0; i < cmds->size(); i++) {
    ...
    case 0xc8: {
      ...
      case LOC_ROAD:
        ...
        break;
      case LOC_RIBBON11:
        DefaultRibbon11Brush = TextureIdToBrush(this, textureid);
        break;
      case LOC_RIBBON12:
        DefaultRibbon12Brush = TextureIdToBrush(this, textureid);
        break;
      case LOC_RIBBON13:
        DefaultRibbon13Brush = TextureIdToBrush(this, textureid);
        break;
      case LOC_RIBBON14:
        DefaultRibbon14Brush = TextureIdToBrush(this, textureid);
        break;
      case LOC_RIBBON15:
        DefaultRibbon15Brush = TextureIdToBrush(this, textureid);
        break;
      case LOC_RIBBON16:
        DefaultRibbon16Brush = TextureIdToBrush(this, textureid);
        break;
      }
    } break;
    }
...
void GPTrack::createTrack(CDocument *pDoc)
{
  ...
  for (int i = 0; i < size; i++) {
    ...
    t->setAltitude(static_cast<int>(GradientAtBeginning));
    ...
    if (height != 0) {
      ...
      ad = 2.0 * length * 180.0 / PI / m * sin(Deg2Rad(m / 2.0)) * sin(Deg2Rad(m / 2.0 + GradientAtBeginning));
...
BOOL GPTrack::PitLaneEntranceToLeft()
{
  ...
  if (a1First && a2Second) {
    ...
    AfxMessageBox(
      "Error with pitlane entrance commands cannot find 0xa1 or 0xa2 "
      "commands");
    return FALSE;
  }
}
...
BOOL GPTrack::PitLaneExitToLeft()
{
  ...
  if (a4First && a3Second) {
    ...
    AfxMessageBox(
      "Error with pitlane exit commands cannot find 0xa3 or 0xa4 commands");
    return FALSE;
  }
}

TrackSection *
  GPTrack::GetTrackSection(int idx)
{
...
TrackSection *
  GPTrack::GetTrackSection(Vector *vec, int idx)
{
  if (vec != NULL) {
...
TrackSection *
  GPTrack::GetNextSection(TrackSection *t)
{
  Vector *vec = t->getVector();
...
CCLineSection *
  GPTrack::GetNextSection(CCLineSection *t)
{
  Vector *vec = t->getVector();
  if (vec) {
...
Camera *
  GPTrack::GetPreviousCamera(Camera *t)
{
...
CCLineSection *
  GPTrack::GetPreviousSection(CCLineSection *t)
{
  Vector *vec = t->getVector();
  if (vec) {
    int idx = GetCCSectionIndex(t);
...
TrackSection *
  GPTrack::GetPreviousSection(TrackSection *t)
{
  Vector *vec = t->getVector();
  if (vec) {
...
TrackSection *
  GPTrack::FindTrackCommand(int cmd, int pos)
{
...
void GPTrack::createPitlane()
{
  ...
  TrackSection *pitlaneStart =
    (TrackSection *)TrackSections->elementAt(PitLaneEntryIndex);
...
void GPTrack::drawGrid(Display *g)
{
  ...
  if (showGraphPaper) {
    ...
    case TEN_METERS:
      gridx = (int)(10.0 / LENGTH_TO_KM);
      break;
    case FIFTY_METERS:
      gridx = (int)(50.0 / LENGTH_TO_KM);
      break;
    case ONE_HUNDRED_METERS:
      gridx = (int)(100.0 / LENGTH_TO_KM);
      break;
    case FIVE_HUNDRED_METERS:
      gridx = (int)(500.0 / LENGTH_TO_KM);
      break;
    case ONE_KM:
      gridx = (int)(1000.0 / LENGTH_TO_KM);
      break;
    case HALF_MILE:
      gridx = (int)(800.0 / LENGTH_TO_KM);
      break;
    default:
    case ONE_MILE:
      gridx = (int)(1600.0 / LENGTH_TO_KM);
      break;
    }
    ...
    for (int i = y; i < y + height; i += gridy) {
...
#define CONTAIN_PITCMD(x) \
  if (t->containsCommand(x)) drawCmdText(g, t, x);
#define CONTAIN_MARKING_CMD(x) \
  if (t->containsCommand(x)) drawMarking(g, t, x);
...
void GPTrack::drawTrack(Display *g, BOOL drawGray)
{
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (showScenery) t->drawScenery(g);
...

  {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (showOutlines) DrawObjectOutline(g, t);

      if (showRoadSigns) t->drawRoadSign(g);

      if (showCommands) t->drawCommands(g);

      if (showTextureMappingCommands) t->drawTextureCommands(g);

      if (showViewDistanceCommands) t->drawViewDistanceCommands(g);

      if (i == 0 && showFinishLine) t->drawStartingLine(g);

      if (showGrip) t->drawGrip(g, t->getAngle(), this);

      if (showPitCmds) drawPitCmds(g);

      if (showMarkings) drawTrackMarkings(g);

      if (showUnkVariousTable) drawVarious(g);
      iter = TrackSections->nextIter(iter);
    }
  }
}
...
double
  GPTrack::LengthToTrackSector(TrackSection *in)
{
  ...
  for (int i = 0; i < size; i++) {
    TrackSection *t = (TrackSection *)TrackSections->elementAt(i);
    if (t == in) return totalLength;

    if (t->getAngle() == 0)
      totalLength += t->getLength();
    else
      totalLength += abs(t->getLength());
  }
  return totalLength;
}

double
  GPTrack::LengthToTrackSectorEnd(TrackSection *in)
{
  double totalLength = 0;
...
double
  GPTrack::LengthToPitSectorEnd(TrackSection *in)
{
  double totalLength = 0;
...
TrackSection *
  GPTrack::SectorForLength(double len)
{
  double totalLength = 0;
  int size = TrackSections->size();
...
void GPTrack::drawCCLine(Display *g)
{
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (showCCLine) t->drawLine(g, &xpos, &ypos, &currangle, ypos);
...
void GPTrack::drawPitlane(Display *g)
{
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (showPitLane) t->drawTrack(g, angle, this);
...
void GPTrack::selectedCCSection(CPoint pt)
{
  ...
  if (selectedSection != NULL) {
    ...
    if (pDoc) {
      pDoc->getTree()->Expand(
        pDoc->getTree()->GetParentItem(selectedSection->getTreeNode()),
        TVE_EXPAND);
...
static int trackDescrs = sizeof(AllTracks) / sizeof(AllTracks[0]);
...
void GPTrack::GuessTrackIdenty(BOOL complain)
{
  ...
  if (!foundTrack && complain) {
    char mess[256];
    wsprintf(mess,
      "The track you have loaded has no GP2INFO added and is not "
      "regocnised by Track Editor as a registered track and its name "
      "could not be found this track has the Magic Number %ld",
      val);
    AfxMessageBox(mess, MB_OK);
  }
}
...
Camera *
  GPTrack::selectedCamera(CPoint pt)
{
  ...
  if (CurrentSelectedCamera == NULL)
    AfxMessageBox("Select Camera Failed");
  else
    CurrentSelectedCamera->setSelected(TRUE);
  return CurrentSelectedCamera;
}

Camera *
  GPTrack::GetSelectedCamera()
{
  for (int i = 0; i < TrackCameras->size(); i++) {
    Camera *t = (Camera *)TrackCameras->elementAt(i);
...
TrackObject *
  GPTrack::selectedObject(CPoint pt)
{
  double minDistance = 10000;
  ...
  for (int i = 0; i < TrackSections->size(); i++) {
    ...
    // could be dangerous
    if (SelObjects) delete (SelObjects);
  }
  ...
  if (CurrentSelectedObject != NULL) CurrentSelectedObject->setSelected(TRUE);

  return CurrentSelectedObject;
}

TrackObject *
  GPTrack::GetSelectedObject()
{
  int size = TrackSections->size();
  for (int i = 0; i < size; i++) {
    TrackSection *t = (TrackSection *)TrackSections->elementAt(i);
...
TrackObjectDefinition *
  GPTrack::AddNewTrackObjectDefinition()
{
  int size = TrackObjectDefintions->size();
  TrackObjectDefinition *last =
    (TrackObjectDefinition *)TrackObjectDefintions->elementAt(size - 1);
  ...
  // AfxMessageBox("Track Object Definitions cannot be saved yet!",MB_OK |
  // MB_ICONWARNING);
  return objdef;
}
...
Model3D *
  GPTrack::createSceneryModel(Display *g)
{
  ...
  for (int i = 0; i < ts; i++) {
    ...
    for (int k = 0; k < cmds->size(); k++) {
      ...
      case 0xB9: {
        ...
        if (value & myribbon) ribbon = TRUE;
      } break;
      case 0xB0: {
        int value = cmd->getNthArg(2);
        // int myribbon = IntToRibbon(j);
        if (value & myribbon) ribbon = FALSE;
      } break;
      }
    }
  }
  ...
  while (!finished) {
    ...
    if (cat && strcmp(cat, "") && strlen(cat) > 0) {
      ...
      if (InternalObjectOffsets && objId >= 0 && objId <= InternalObjectOffsets->size()) {
        InternalObject *obj =
          (InternalObject *)InternalObjectOffsets->elementAt(objId);
        obj->setObjectName(_strdup(val));
        obj->MagicNumber = 0;
...
public:
  ...
  ~glPolygonItem() { pts->removeAllData(); };
  Vector *pts;
  double r, g, b;
};
...
void vmlEnd(std::ofstream &file)
{
  ...
  // file << "coordIndex [ 0 1 2 3 0]" << std::endl;
  file << "coordIndex [ ";
  for (int i = 0; i < currentPolygon->pts->size(); i++) {
    file << " " << i;
  }
...
void GPTrack::CreateVRMLFile(char *filename)
{
  ...
  if (mytrack->showTrack) {
    ...
    while (t) {
      ...
      double rightSideX =
        xpos + getRightSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousRight));
      double rightSideY =
        ypos + getRightSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousRight));
      double KleftSideX =
        xpos + getLeftSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousLeft - KWIDTH));
      double KleftSideY =
        ypos + getLeftSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousLeft - KWIDTH));
      double KrightSideX =
        xpos + getRightSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousRight - KWIDTH));
      double KrightSideY =
        ypos + getRightSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousRight - KWIDTH));

      double Kerb1leftSideX =
        xpos + getLeftSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousLeft + KERB_1_WIDTH));
      double Kerb1leftSideY =
        ypos + getLeftSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousLeft + KERB_1_WIDTH));
      double Kerb1rightSideX =
        xpos + getRightSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousRight + KERB_1_WIDTH));
      double Kerb1rightSideY =
        ypos + getRightSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousRight + KERB_1_WIDTH));

      double Kerb2leftSideX =
        xpos + getLeftSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousLeft + KERB_2_WIDTH));
      double Kerb2leftSideY =
        ypos + getLeftSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousLeft + KERB_2_WIDTH));
      double Kerb2rightSideX =
        xpos + getRightSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousRight + KERB_2_WIDTH));
      double Kerb2rightSideY =
        ypos + getRightSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousRight + KERB_2_WIDTH));

      // track parts
      double leftSideNX =
        nextxpos + getLeftSideX(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthLeft()));
      double leftSideNY =
        nextypos + getLeftSideY(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthLeft()));
      double rightSideNX =
        nextxpos + getRightSideX(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthRight()));
      double rightSideNY =
        nextypos + getRightSideY(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthRight()));
      double KleftSideNX =
        nextxpos + getLeftSideX(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthLeft() - KWIDTH));
      double KleftSideNY =
        nextypos + getLeftSideY(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthLeft() - KWIDTH));
      double KrightSideNX =
        nextxpos + getRightSideX(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthRight() - KWIDTH));
      double KrightSideNY =
        nextypos + getRightSideY(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthRight() - KWIDTH));

      double Kerb1leftSideNX =
        nextxpos + getLeftSideX(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthLeft() + KERB_1_WIDTH));
      double Kerb1leftSideNY =
        nextypos + getLeftSideY(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthLeft() + KERB_1_WIDTH));
      double Kerb1rightSideNX =
        nextxpos + getRightSideX(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthRight() + KERB_1_WIDTH));
      double Kerb1rightSideNY =
        nextypos + getRightSideY(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthRight() + KERB_1_WIDTH));
      double Kerb2leftSideNX =
        nextxpos + getLeftSideX(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthLeft() + KERB_2_WIDTH));
      double Kerb2leftSideNY =
        nextypos + getLeftSideY(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthLeft() + KERB_2_WIDTH));
      double Kerb2rightSideNX =
        nextxpos + getRightSideX(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthRight() + KERB_2_WIDTH));
      double Kerb2rightSideNY =
        nextypos + getRightSideY(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthRight() + KERB_2_WIDTH));
      ...
      // track parts
      double WleftSideX =
        xpos + getLeftSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(t->getWidthLeft() + leftWall));
      double WleftSideY =
        ypos + getLeftSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(t->getWidthLeft() + leftWall));
      double WrightSideX =
        xpos + getRightSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(t->getWidthRight() + rightWall));
      double WrightSideY =
        ypos + getRightSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(t->getWidthRight() + rightWall));

      // track parts
      double WleftSideNX =
        nextxpos + getLeftSideX(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthLeft() + EleftWall));
      double WleftSideNY =
        nextypos + getLeftSideY(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthLeft() + EleftWall));
      double WrightSideNX =
        nextxpos + getRightSideX(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthRight() + ErightWall));
      double WrightSideNY =
        nextypos + getRightSideY(sectorStartAngleN, WIDTH_TO_TRACKWIDTH(t->getWidthRight() + ErightWall));
      ...
      if (mytrack->showTextures) {
        // glEnable(GL_TEXTURE_2D);
        // VergeRightBits=
        // TextureMap(GetDC(),VergeRightBmp,VergeRightBits,FALSE);
      }
      ...
      if (mytrack->showWalls) {
        if (mytrack->showTextures) {
          // glEnable(GL_TEXTURE_2D);
          // FenceLeftBits =
          // TextureMap(GetDC(),FenceLeftBmp,FenceLeftBits,FALSE);
        }
        ...
        if (mytrack->showTextures) {
          // glEnable(GL_TEXTURE_2D);
          // FenceRightBits =
          // TextureMap(GetDC(),FenceRightBmp,FenceRightBits,FALSE);
        }
      ...
      if (t->isRightKerb()) {
        ...
        vmlTexCoord2f(1.0f, 1.0f);
        vmlVertex3d(GET_GL_PT_X(Kerb1rightSideNX),
          GET_GL_PT_Y(Kerb1rightSideNY),
          GET_GL_PT_Z(nextzpos - KERB_1_HEIGHT));
        vmlTexCoord2f(1.0f, 0.0f);
        vmlVertex3d(GET_GL_PT_X(rightSideNX), GET_GL_PT_Y(rightSideNY), GET_GL_PT_Z(nextzpos));
        vmlEnd(file);
        ...
        vmlTexCoord2f(1.0f, 1.0f);
        vmlVertex3d(GET_GL_PT_X(Kerb2rightSideNX),
          GET_GL_PT_Y(Kerb2rightSideNY),
          GET_GL_PT_Z(nextzpos - KERB_2_HEIGHT));
        vmlTexCoord2f(1.0f, 0.0f);
        vmlVertex3d(GET_GL_PT_X(Kerb1rightSideNX),
          GET_GL_PT_Y(Kerb1rightSideNY),
          GET_GL_PT_Z(nextzpos - KERB_1_HEIGHT));
        ...
        for (int i = 0; i < numObjects; i++) {
          ...
          if (t != NULL) {
            ...
            if (angle != 0) {
              ...
              nextxpos = trackPt.y;
              double deltaAngle =
                DegToRad(angle * length) * (remainder / length);
              angle = stAngle + deltaAngle;
            } else {
              nextypos = ob->getDistance() * sin(stAngle);
              nextxpos = ob->getDistance() * cos(stAngle);
  ...
  if (mytrack->showCameras) {
    for (int i = 0; i < mytrack->TrackCameras->size(); i++) {
      ...
      CPosAngle *CameraPos =
        DistanceToPoint(mytrack, (cam->getDistance() - 16384));
  ...
  file << "NavigationInfo { type \"EXAMINE\" }" << std::endl;
  file << "Viewpoint { description \"start\" position 0 0 1000 } " << std::endl;
  file.close();
}
