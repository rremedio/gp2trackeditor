...
void CTrackSelection::OnTrackAdd()
{
  ...
  if (list) inStallTo = list->GetCurSel();
  ...
  if (originalFile == NULL) AfxMessageBox("No To File");

  if (newFile == NULL) AfxMessageBox("No From File");
  ...
  // char buffer[256];
  // wsprintf(buffer,"Copying track %d to track %d please
  // confirm?",inStallFrom,inStallTo);
  ...
  if (originalFile && newFile) {
    ...
    // wsprintf(buffer,"Copying track %s to track %s please
    // confirm?",originalFile,originalFileOrig); result =
    // AfxMessageBox(buffer,MB_YESNO); if (result == IDNO) return;

    if (CopyFile(originalFile, originalFileOrig, TRUE) == FALSE) {
      // result = AfxMessageBox("A Backup Copy of the original file already
      // exists to you want to overwrite the backup",MB_YESNO); if (result ==
      // IDYES)
      //{
    ...
    // result = AfxMessageBox("About to Install Track Are you still
    // sure?",MB_YESNO); if (result == IDNO) return;
...
{
public:
  SeasonFile(LPCSTR file, int pos) : filePosition(pos)
  {
    fileName = _strdup(file);
  }

  virtual ~SeasonFile() { free(fileName); }
...
void OpenSeasonSetFile(LPCSTR filename)
{
  ...
  const char *SeasonSetYear = NULL;
  const char *SeasonSetEvent = NULL;
  ...
  if (result == IDYES) {
    for (int i = 0; i < TrackFiles->size(); i++) {
      ...
      if (nextFile) {
        // wsprintf(buffer,"Installing %s to
        // F1ct%2.2d.dat",nextFile->fileName,nextFile->filePosition);
        // AfxMessageBox(buffer);
        char message[1024];
