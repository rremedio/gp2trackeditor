...
BOOL CTrackEditorApp::InitInstance()
{
  ...
  pDocTemplate = new CMultiDocTemplate(
    IDR_TRACKETYPE, RUNTIME_CLASS(CTrackEditorDoc),
  ...
  pDocTableTemplate = new CMultiDocTemplate(
    IDR_TRACKDATA, RUNTIME_CLASS(CTrackEditorDoc), RUNTIME_CLASS(CChildFrame), RUNTIME_CLASS(CTrackInfo));
  AddDocTemplate(pDocTableTemplate);

  pCCTemplate =
    new CMultiDocTemplate(IDR_CCDATA, RUNTIME_CLASS(CTrackEditorDoc), RUNTIME_CLASS(CChildFrame), RUNTIME_CLASS(CInfo));
  AddDocTemplate(pCCTemplate);

  pPitTemplate = new CMultiDocTemplate(
    IDR_PITDATA, RUNTIME_CLASS(CTrackEditorDoc), RUNTIME_CLASS(CChildFrame), RUNTIME_CLASS(CPitInfo));
  AddDocTemplate(pPitTemplate);

  pObjectTemplate = new CMultiDocTemplate(
    IDR_OBJECT, RUNTIME_CLASS(CTrackEditorDoc), RUNTIME_CLASS(CChildFrame), RUNTIME_CLASS(CObjectInfo));
  AddDocTemplate(pObjectTemplate);

  pCameraTemplate = new CMultiDocTemplate(
    IDR_CAMERA, RUNTIME_CLASS(CTrackEditorDoc), RUNTIME_CLASS(CChildFrame), RUNTIME_CLASS(CCameraView));
  AddDocTemplate(pCameraTemplate);

  pObjectViewTemplate = new CMultiDocTemplate(
    IDR_OBJECT_INTERNAL, RUNTIME_CLASS(CTrackEditorDoc), RUNTIME_CLASS(CChildFrame), RUNTIME_CLASS(CObjectView));
  AddDocTemplate(pObjectViewTemplate);

  p3DViewTemplate = new CMultiDocTemplate(
    IDR_WORLD, RUNTIME_CLASS(CTrackEditorDoc), RUNTIME_CLASS(CChildFrame), RUNTIME_CLASS(CWorldView));
  AddDocTemplate(p3DViewTemplate);

  pCommandStatsTemplate = new CMultiDocTemplate(
    IDR_TRACKDATA, RUNTIME_CLASS(CTrackEditorDoc), RUNTIME_CLASS(CChildFrame), RUNTIME_CLASS(CCommandStats));
  AddDocTemplate(pCommandStatsTemplate);

  pJamFileTemplate = new CMultiDocTemplate(
    IDR_TRACKDATA, RUNTIME_CLASS(CTrackEditorDoc), RUNTIME_CLASS(CChildFrame), RUNTIME_CLASS(CJamView));
  AddDocTemplate(pJamFileTemplate);

  pInternalFileTemplate = new CMultiDocTemplate(
    IDR_TRACKDATA, RUNTIME_CLASS(CTrackEditorDoc), RUNTIME_CLASS(CChildFrame), RUNTIME_CLASS(CInternalView));
  AddDocTemplate(pInternalFileTemplate);

  pTrackGLViewTemplate = new CMultiDocTemplate(
    IDR_TRACKDATA, RUNTIME_CLASS(CTrackEditorDoc), RUNTIME_CLASS(CChildFrame), RUNTIME_CLASS(CTrackOpenGLView));
  ...
  placement.flags =
    AfxGetApp()->GetProfileInt(strSection, "SizeFlags", placement.flags);
  placement.showCmd =
    AfxGetApp()->GetProfileInt(strSection, "SizeFlags", placement.showCmd);
  placement.ptMinPosition.x = AfxGetApp()->GetProfileInt(
    strSection, "SizeptMinPosX", placement.ptMinPosition.x);
  placement.ptMinPosition.y = AfxGetApp()->GetProfileInt(
    strSection, "SizeptMinPosY", placement.ptMinPosition.y);
  placement.ptMaxPosition.x = AfxGetApp()->GetProfileInt(
    strSection, "SizeptMaxPosX", placement.ptMaxPosition.x);
  placement.ptMaxPosition.y = AfxGetApp()->GetProfileInt(
    strSection, "SizeptMaxPosY", placement.ptMaxPosition.y);
  placement.rcNormalPosition.left = AfxGetApp()->GetProfileInt(
    strSection, "SizeLeft", placement.rcNormalPosition.left);
  placement.rcNormalPosition.top = AfxGetApp()->GetProfileInt(
    strSection, "SizeTop", placement.rcNormalPosition.top);
  placement.rcNormalPosition.right = AfxGetApp()->GetProfileInt(
    strSection, "SizeRight", placement.rcNormalPosition.right);
  placement.rcNormalPosition.bottom = AfxGetApp()->GetProfileInt(
    strSection, "SizeBottom", placement.rcNormalPosition.bottom);
...
CDocument*
  CTrackEditorApp::OpenDocumentFile(LPCTSTR lpszFileName)
{
...
CObjectView*
  CTrackEditorApp::OnInternalObjectView(CDocument* pDoc)
{
...
void CTrackEditorApp::OnFileNew()
{
  ...
  // TODO: Add your command handler code here
  CString msg =
    "The TrackEditor works by editing exiting cirucits only. Simply select a "
    "circuit from the GP2/circuits directroy which you wish to use as your "
    "base circuit, edit this circuit then resave to the GP2/circuits "
    "directory under the original name.\n\n(be sure to backup this circuit "
    "before saving)";
  msg +=
    "\nThe TrackEditor assumes you have installed NOCDROM or TracksOnHD "
    "(availabe from www.grandprix2.com) and you have already copied your "
    "tracks from the GP2 cdrom so that they are on your disk (where they can "
    "be modified once they are changed from readonly!)\n";
  msg +=
    "\nTracks must be save at f1ctXX.dat (where XX is a number 01-16, "
    "denoting which coutry to replace (01 = Brazil etc..). Also sure to "
    "ensure you have backup up your GP2 directory.";
...
void CTrackEditorApp::OnViewToolsToolbar()
{
  ...
  pMainFrame->m_wndToolsToolBar.ShowWindow(showToolsToolbar ? SW_SHOW
                                                            : SW_HIDE);
  pMainFrame->RecalcLayout(TRUE);
}
...
extern void
  ImportObjectDefinitions(CString name);
...
void CTrackEditorApp::OnImportNames()
{
  ...
  CFileDialog* fdlg =
    new CFileDialog(TRUE, "txt", (LPCSTR)&fileName, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, lpszObjOffFilter);
...
void SaveControlBarPosition(CFrameWnd* frame, CControlBar* cntrl, CString strStringItem)
{
  ...
  //((CToolBar
  //*)cntrl)->GetToolBarCtrl().SaveState(HKEY_CURRENT_USER,"Software\\GP2TrackEditorProject\\TrackEditor",strSection);
...
void LoadControlBarPosition(CFrameWnd* frame, CControlBar* cntrl, CString strStringItem)
{
  ...
  //((CToolBar
  //*)cntrl)->GetToolBarCtrl().RestoreState(HKEY_CURRENT_USER,"Software\\GP2TrackEditorProject\\TrackEditor",strSection);
  ...
  if (floating) {
    ...
    if (width > 5 && height > 5) {
      ...
      // wnd->SetWindowPos(NULL,0,0,width+5,height+20,SWP_NOMOVE | SWP_NOZORDER
      // | SWP_FRAMECHANGED);
      // cntrl->SetWindowPos(NULL,pt.x,pt.y,width,height,SWP_NOMOVE |
      // SWP_NOZORDER | SWP_FRAMECHANGED);
      // cntrl->GetParent()->SetWindowPos(NULL,pt.x,pt.y,width,height,SWP_NOMOVE
      // | SWP_NOZORDER | SWP_FRAMECHANGED);
      //((CToolBar *)cntrl)->SetHeight(height);
      //((CToolBar
      //*)cntrl)->GetToolBarCtrl().SetRows((int)(height/20),TRUE,NULL);
...
void CTrackEditorApp::OnStartHelp(BOOL reposition)
{
  ...
  if (reposition)
    typeInfo.fsValidMembers |= (HHWIN_PARAM_NAV_WIDTH | HHWIN_PARAM_RECT);
...
void CTrackEditorApp::OnViewObjecttoolbar()
{
  ...
  pMainFrame->m_wndObjectToolBar.ShowWindow(showObjectToolbar ? SW_SHOW
                                                              : SW_HIDE);
  pMainFrame->RecalcLayout(TRUE);
}
...
{
        //this code excerpt also demonstrates how to use //try/catch exception
handling
...
pServer->OpenRequest(CHttpConnection::HTTP_VERB_GET, strObject);
                    ...
                    //catch errors from WinInet
                        //psz->ReportError();
                        AfxMessageBox("Unable to check online connection
error"); session.Close(); return FALSE;
                }
                session.Close();
...
BOOL CTrackEditorApp::CheckCurrentEditorVersion()
{
  ...
  if (cmonth != month || cday != day || timeChecked >= 10) {
    ...
    timeChecked = 0;
    WriteProfileInt("Upgrade", "CheckTimes", timeChecked);
    // cerr << "Checking with online version..." << std::endl;
    /*
if (GetVersionPage(Major,Minor,Debug))
    {
      ...
      if (upgrade)
      {
              ...
              msg += " Shall I open Your browser so you can download the next
version?"; int result = AfxMessageBox(msg,MB_YESNO); if (result == IDYES)
              {
                      ShellExecute(NULL,"open",
"http://members.xoom.com/paulhoad/index.html",NULL, NULL, SW_SHOWNORMAL);
              }
              else
              {
      ...
      {
              //cerr << "You have an upto date version" << std::endl;
      }
    }
    */
    return TRUE;
  } else {
    // cerr << "Version check upto date [" << timeChecked << "]" << std::endl;
    return FALSE;
  }
}
...
bool CTrackEditorApp::IsLandScape()
{
  ...
  if (GetPrinterDeviceDefaults(&pd)) {
    ...
    if (pDevMode) {
      ...
      ::DeviceCapabilities(lpszDeviceName, lpszPortName, DC_ORIENTATION, 0L, pDevMode);
...
static void visitURL(LPCSTR linkText)
{
  HINSTANCE h = ShellExecute(NULL, (LPCTSTR) "open", (LPCSTR)linkText, NULL, NULL, SW_SHOWNORMAL);
