...
extern void
  drawSingleSide(Display *g, TrackSection *t1, int beginOffset, int length, int side);
...
Vector *
  TrackSection::getCommandIterator()
{
...
void TrackSection::drawStartingLine(Display *g)
{
  ...
  double leftSideX =
    xpos + getLeftSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft()) + 2);
  double leftSideY =
    ypos + getLeftSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft()) + 2);

  double rightSideX =
    xpos + getRightSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(getWidthRight()) + 2);
  double rightSideY =
    ypos + getRightSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(getWidthRight()) + 2);

  // track parts
  double leftSideNX =
    xpos + nextxpos + getLeftSideX(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft()) + 2);
  double leftSideNY =
    ypos + nextypos + getLeftSideY(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft()) + 2);
...
void TrackSection::drawFlag(Display *g, BOOL right)
{
  ...
  double rightSideX =
    xpos + getRightSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(getWidthRight()));
  double rightSideY =
    ypos + getRightSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(getWidthRight()));
...
CPoint
  TrackSection::TrackNextPoint(double xpos, double ypos, double angle, double stAngle, int inDistance)
{
  CPoint retPt;
...
void TrackSection::drawRoadSign(Display *g)
{
  ...
  CPoint Sign1 =
    TrackNextPoint(prev->getStartX(), prev->getStartY(), prev->getAngle(), prev->getStartAngle(), length - 24);
  CPoint Sign2 =
    TrackNextPoint(prev->getStartX(), prev->getStartY(), prev->getAngle(), prev->getStartAngle(), length - 16);
  CPoint Sign3 =
    TrackNextPoint(prev->getStartX(), prev->getStartY(), prev->getAngle(), prev->getStartAngle(), length - 8);
  ...
  if (hasRoadSigns()) {
    ...
    if (sign1 != IDB_UNK) g->drawBitmap(sign1, Sign1.x, Sign1.y);
    if (sign2 != IDB_UNK) g->drawBitmap(sign2, Sign2.x, Sign2.y);
    if (sign3 != IDB_UNK) g->drawBitmap(sign3, Sign3.x, Sign3.y);
  }
}
...
Primitive3D *
  TrackSection::drawTrack3D(Display *g, double angleNotUsed, GPTrack *track, TrackSection *t1, TrackSection *t2)
{
  ...
  // track parts
  double leftSideNX =
    xpos + nextxpos + getLeftSideX(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft()));
  double leftSideNY =
    ypos + nextypos + getLeftSideY(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft()));

  double rightSideNX =
    xpos + nextxpos + getRightSideX(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthRight()));
  double rightSideNY =
    ypos + nextypos + getRightSideY(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthRight()));
  ...
  // track parts
  leftSideX =
    xpos + getLeftSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft() + leftWall));
  leftSideY =
    ypos + getLeftSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft() + leftWall));

  rightSideX =
    xpos + getRightSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(getWidthRight() + rightWall));
  rightSideY =
    ypos + getRightSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(getWidthRight() + rightWall));
  ...
  rightSideNX =
    xpos + nextxpos + getRightSideX(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthRight() + ErightWall));
  rightSideNY =
    ypos + nextypos + getRightSideY(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthRight() + ErightWall));

  if (track->showTrackSide) {
    // track parts
    double TleftSideX =
      xpos + getLeftSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft()));
    double TleftSideY =
      ypos + getLeftSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft()));
    double TleftSideNX =
      xpos + nextxpos + getLeftSideX(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft()));
    double TleftSideNY =
      ypos + nextypos + getLeftSideY(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft()));
    ...
    double WleftSideY =
      ypos + getLeftSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft() + leftWall));
    double WleftSideNX =
      xpos + nextxpos + getLeftSideX(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft() + EleftWall));
    double WleftSideNY =
      ypos + nextypos + getLeftSideY(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthLeft() + EleftWall));
...
void TrackSection::drawTrack(Display *g, double angle, GPTrack *track, BOOL drawGray)
{
  ...
  // track parts
  double leftSideX =
    xpos + getLeftSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousLeft));
  double leftSideY =
    ypos + getLeftSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(previousLeft));
  ...
  trackPolygon->addPoint(g->getScreenX(rightSideNX),
    g->getScreenY(rightSideNY));
...

    {
      // if (track->showFilledObjects)
      {
        ...
        if (getAngle() < 0) {
          ...
          // g->drawArcSpecial((xpos+nextxpos),
          // (ypos+nextypos),xpos,ypos,cx,cy,getStartAngle(),getAngle()*getLength(),getRadius()+WIDTH_TO_TRACKWIDTH(getWidthRight()),getRadius()-WIDTH_TO_TRACKWIDTH(getWidthLeft()));
        } else {
          double cx = xpos + getRightSideX(sectorStartAngle, getRadius());
          double cy = ypos + getRightSideY(sectorStartAngle, getRadius());
          double r1 = getRadius() - WIDTH_TO_TRACKWIDTH(previousRight);
          ...
          // g->drawArcSpecial((xpos+nextxpos),
          // (ypos+nextypos),xpos,ypos,cx,cy,getStartAngle(),getAngle()*getLength(),getRadius()-WIDTH_TO_TRACKWIDTH(getWidthLeft()),getRadius()+WIDTH_TO_TRACKWIDTH(getWidthRight()));
        }
      }
      /*
      else
      ...
      (ypos+nextypos),xpos,ypos,cx,cy,getStartAngle(),getAngle()*getLength(),getRadius()-WIDTH_TO_TRACKWIDTH(getWidthLeft()));
        }
        else{
           g->drawArc((xpos+nextxpos),
      (ypos+nextypos),xpos,ypos,cx,cy,getStartAngle(),getAngle()*getLength(),getRadius()-WIDTH_TO_TRACKWIDTH(getWidthRight()));
        }
      }
      */
  ...
  if (track->showAnnotations) {
    ...
    double leftSideX =
      txpos + getLeftSideX(sectorStartAngle, WIDTH_TO_TRACKWIDTH(4000));
    double leftSideY =
      typos + getLeftSideY(sectorStartAngle, WIDTH_TO_TRACKWIDTH(4000));
...
static void
  GetTrackPos(double currrad, double angle, double length, double &xpos, double &ypos, double &endangle)
{
  CPoint pt;
...
void DrawRibbon(GPTrack *track, Display *g, double xpos, double ypos, double x1, double y1, double nx1, double ny1, double x2, double y2, double nx2, double ny2, int textureId, int ribbonNumber)
{
    ...
    for (int i = 0; i < cmds->size(); i++) {
      ...
      if (distanceIn != 0) {
        ...
        if (angle.GetValue() != 0) {
          ...
          newy = ypos + trackPt.x;
          double deltaAngle =
            DegToRad(nangle * getLength()) * (distanceIn / getLength());
      ...
      CreateRibbons(scenery, newx, newy, newz, 0, newangle, nx1, ny1, nz1, nx2, ny2, nz2, nx3, ny3, nz3, nx4, ny4, nz4, nx5, ny5, nz5, nx6, ny6, nz6, 2048, LeftAngle, RightAngle, CurrentScenerySide);
...
void TrackSection::drawSides(Display *g, double angle, GPTrack *track, TrackSection *t1, TrackSection *t2, TrackSection *prev)
{
  ...
  if (getAngle() < 0) {
    ...
    // g->drawArcSpecial((xpos+nextxpos),
    // (ypos+nextypos),xpos,ypos,cx,cy,getStartAngle(),getAngle()*getLength(),getRadius()+WIDTH_TO_TRACKWIDTH(getWidthLeft()+EleftWall),getRadius()+WIDTH_TO_TRACKWIDTH(getWidthLeft()),TRUE);
  } else if (getAngle() > 0) {
    double cx = xpos + getRightSideX(sectorStartAngle, getRadius());
    double cy = ypos + getRightSideY(sectorStartAngle, getRadius());
    ...
    // g->drawArcSpecial((xpos+nextxpos),
    // (ypos+nextypos),xpos,ypos,cx,cy,getStartAngle(),getAngle()*getLength(),getRadius()+WIDTH_TO_TRACKWIDTH(getWidthLeft()),getRadius()+WIDTH_TO_TRACKWIDTH(getWidthLeft()+EleftWall));
  } else {
    // left
  ...
  if (getAngle() < 0) {
    ...
    // g->drawArcSpecial((xpos+nextxpos),
    // (ypos+nextypos),xpos,ypos,cx,cy,getStartAngle(),getAngle()*getLength(),getRadius()-WIDTH_TO_TRACKWIDTH(getWidthRight()),getRadius()-WIDTH_TO_TRACKWIDTH(getWidthRight()+ErightWall));
  } else if (getAngle() > 0) {
    double cx = xpos + getRightSideX(sectorStartAngle, getRadius());
    double cy = ypos + getRightSideY(sectorStartAngle, getRadius());
    double r1 = getRadius() - WIDTH_TO_TRACKWIDTH(getWidthRight() + rightWall);
    ...
    // g->drawArcSpecial((xpos+nextxpos),
    // (ypos+nextypos),xpos,ypos,cx,cy,getStartAngle(),getAngle()*getLength(),getRadius()-WIDTH_TO_TRACKWIDTH(getWidthRight()+ErightWall),getRadius()-WIDTH_TO_TRACKWIDTH(getWidthRight()));
  } else {
    // right
    GPPolygon *edge = new GPPolygon();
    edge->empty();
...
void TrackSection::GetBeginRightWall(double &leftSideX, double &leftSideY)
{
  ...
  double sectorEndAngle = getStartAngle();
  leftSideX =
    xpos + getRightSideX(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthRight() + rightWall));
  leftSideY =
    ypos + getRightSideY(sectorEndAngle, WIDTH_TO_TRACKWIDTH(getWidthRight() + rightWall));
}
...
void TrackSection::drawWalls(Display *g, double angle, GPTrack *track, TrackSection *t1, TrackSection *t2)
{
  ...
  // track parts
  double leftSideNX =
    xpos + nextxpos + getLeftSideX(sectorEndAngleN, WIDTH_TO_TRACKWIDTH(getWidthLeft() + EleftWall));
  double leftSideNY =
    ypos + nextypos + getLeftSideY(sectorEndAngleN, WIDTH_TO_TRACKWIDTH(getWidthLeft() + EleftWall));
  double rightSideNX =
    xpos + nextxpos + getRightSideX(sectorEndAngleN, WIDTH_TO_TRACKWIDTH(getWidthRight() + ErightWall));
  double rightSideNY =
    ypos + nextypos + getRightSideY(sectorEndAngleN, WIDTH_TO_TRACKWIDTH(getWidthRight() + ErightWall));
  ...
  if (angle == 0) {
    ...
    if (!isLeftWallRemoved() || getTrack()->showRemovedWalls && !drawnLeft) {
      if (!track->showCurvedWallStraight) {
        // g->drawArc((xpos+nextxpos),
        // (ypos+nextypos),xpos,ypos,cx,cy,getStartAngle(),getAngle()*getLength(),t2->getRadius()+WIDTH_TO_TRACKWIDTH(getWidthLeft()+leftWall));
        double r1 =
          t2->getRadius() + WIDTH_TO_TRACKWIDTH(getWidthLeft() + leftWall);
        double r2 =
          t2->getRadius() + WIDTH_TO_TRACKWIDTH(getWidthLeft() + EleftWall);
        g->setColor(-1);
        g->setColor(0);
    ...
    if (!isRightWallRemoved() || getTrack()->showRemovedWalls && !drawnRight) {
      if (!track->showCurvedWallStraight) {
        // g->drawArc((xpos+nextxpos),
        // (ypos+nextypos),xpos,ypos,cx,cy,getStartAngle(),getAngle()*getLength(),t2->getRadius()-WIDTH_TO_TRACKWIDTH(getWidthRight()+rightWall));
        double r1 =
          t2->getRadius() - WIDTH_TO_TRACKWIDTH(getWidthRight() + rightWall);
        double r2 =
          t2->getRadius() - WIDTH_TO_TRACKWIDTH(getWidthRight() + ErightWall);
        g->setColor(-1);
        g->setColor(0);
        g->drawSpiral(cx, cy, getStartAngle(), getEndAngle(), r1, r2);
      } else {
      ...
      // int rad1 =
      // t2->getRadius()-WIDTH_TO_TRACKWIDTH(getWidthRight()+ErightWall); int
      // rad2 = t2->getRadius()-WIDTH_TO_TRACKWIDTH(getWidthRight()+rightWall);
      // g->drawArc((xpos+nextxpos),
      // (ypos+nextypos),xpos,ypos,cx,cy,getStartAngle(),getAngle()*getLength(),t2->getRadius()-WIDTH_TO_TRACKWIDTH(getWidthRight()+rightWall));
      // g->setColor(-1);
...
{
  if (getKerbData() & 0x08) return TRUE;
  return FALSE;
}

BOOL TrackSection::isRightKerb()
{
  if (getKerbData() & 0x04) return TRUE;
  return FALSE;
}
...
{
  if (getKerbData() & 0x20) return TRUE;
  return FALSE;
}

BOOL TrackSection::isRightWallRemoved()
{
  if (getKerbData() & 0x10) return TRUE;
  return FALSE;
}
...
BOOL TrackSection::hasRoadSigns()
{
  ...
  if (getRoadSignData() & 0x8 && getRoadSignData() & 0x80)
    return FALSE;
  else if (getRoadSignData() & 0x8 && getRoadSignData() & 0x80 && getRoadSignData() & 0x40)
    return FALSE;
  else if (getRoadSignData() & 0x8)
    return TRUE;
  else if (getRoadSignData() & 0x80)
    return TRUE;
  else if (getRoadSignData() & 0x40)
    return TRUE;
...
int TrackSection::write(GPTrack *track, int myoffset)
{
  ...
  // offset=-4
  track->WritePCByte(myoffset, lengthData);
  myoffset += 1;
  // cmd
  track->WritePCByte(myoffset, 0x00);
  myoffset += 1;
...
TrackCmd *
  TrackSection::getCommand(int cmd)
{
  int size = TrackCommands->size();
  for (int i = 0; i < size; i++) {
...
Vector *
  TrackSection::getCommands(int cmd)
{
...
extern void
  LoadCmdListBox(CListBox *list);
extern int
  getCmdNumArgs(int id);
...
void TrackSection::LoadPropertyTable(CTrackPropertySheet *table)
{
  ...
  DataChangeObserver *angleobserver = new DataChangeObserver(
    getTrack(), &angle, t_INT, "Angle", "Angle of Section");
  ...
  DataChangeObserver *heightobserver = new DataChangeObserver(
    getTrack(), &heightDelta, t_INT, "Height", "Height Delta of Section");
  ...
  DataChangeObserver *brobserver =
    new DataChangeObserver(getTrack(), &trackToWallWidthRight, t_INT, "Bank Right", "Track To Bank Right");
  ...
  DataChangeObserver *blobserver =
    new DataChangeObserver(getTrack(), &trackToWallWidthLeft, t_INT, "Bank Left", "Track To Bank Left");
...
{
  TrackCmd *gravel =
    new TrackCmd(track, this, 0xBC, 0, "GRAVEL", 0, 9, getLength(), track->DefaultGravelBrush, 3, 16, 0, 3);
  addCommand(gravel);
}
...
{
  TrackCmd *gravel =
    new TrackCmd(track, this, 0xBC, 0, "GRAVEL", 0, 8, getLength(), track->DefaultGravelBrush, 3, 16, 0, 35);
  addCommand(gravel);
}
...
{
  TrackCmd *gravel =
    new TrackCmd(track, this, 0xBC, 0, "TYPEWALL", 0, 5, getLength(), track->DefaultTyreWallBrush, 3, 16, 0, 3);
  addCommand(gravel);
}
...
{
  TrackCmd *gravel =
    new TrackCmd(track, this, 0xBC, 0, "TYPEWALL", 0, 4, getLength(), track->DefaultTyreWallBrush, 3, 16, 0, 35);
  addCommand(gravel);
}
...
{
  TrackCmd *gravel =
    new TrackCmd(track, this, 0xBC, 0, "TARMAC", 0, 9, getLength(), track->DefaultTarmacBrush, 3, 16, 0, 3);
  addCommand(gravel);
}
...
{
  TrackCmd *gravel =
    new TrackCmd(track, this, 0xBC, 0, "TARMAC", 0, 8, getLength(), track->DefaultTarmacBrush, 3, 16, 0, 35);
  addCommand(gravel);
}
...
{
  TrackCmd *gravel =
    new TrackCmd(track, this, 0xBC, 0, "MARKING", offset, location, length, texture, unk1, unk2, unk3, rotation);
  addCommand(gravel);
}
...
void TrackSection::AddBlackFlagRight(int speed)
{
  ...
  CGeneralDataEnry *dataEntry =
    new CGeneralDataEnry("Black Flag RHS Data Entry", "Start Pos", lengthToBegin, "End Pos", endLength, "Speed", 150);

  if (dataEntry->ProcessEntry()) {
    TrackCmd *flag =
      new TrackCmd(track, this, 0xDF, 0, "FLAG", 0, dataEntry->m_EditVal1, dataEntry->m_EditVal2, dataEntry->m_EditVal3);
...
void TrackSection::AddBlackFlagLeft(int speed)
{
  ...
  CGeneralDataEnry *dataEntry =
    new CGeneralDataEnry("Black Flag LHS Data Entry", "Start Pos", lengthToBegin, "End Pos", endLength, "Speed", 150);

  if (dataEntry->ProcessEntry()) {
    TrackCmd *flag =
      new TrackCmd(track, this, 0xDE, 0, "FLAG", 0, dataEntry->m_EditVal1, dataEntry->m_EditVal2, dataEntry->m_EditVal3);
    begin->addCommand(flag);
  }
}
...
Polygon3D *
  DrawRibbon3D(GPTrack *track, Display *g, double xpos, double ypos, double zpos, double x1, double y1, double z1, double nx1, double ny1, double nz1, double x2, double y2, double z2, double nx2, double ny2, double nz2, int textureId)
{
  if (x1 != -1 && x2 != -1 && nx1 != -1 && nx2 != -1) {
...
Model3D *
  TrackSection::drawScenery3D(Display *g)
{
  double xpos = getStartX();
  double ypos = getStartY();
  double zpos = getStartY();
  ...
  if (cmds != NULL && cmds->size() > 0) {
    ...
    for (int i = 0; i < cmds->size(); i++) {
      ...
      CreateRibbons(scenery, newx, newy, newz, 0, newangle, nx1, ny1, nz1, nx2, ny2, nz2, nx3, ny3, nz3, nx4, ny4, nz4, nx5, ny5, nz5, nx6, ny6, nz6, 2048, 0, 0, t_BothScenery);

      Polygon3D *poly1 =
        DrawRibbon3D(track, g, xpos, ypos, zpos, x1, y1, z1, nx1, ny1, nz1, x2, y2, z2, nx2, ny2, nz2, track->DefaultRibbon11Brush);
      if (poly1) scenerySegments->addPrimitive(poly1);
      Polygon3D *poly2 =
        DrawRibbon3D(track, g, xpos, ypos, zpos, x5, y5, z5, nx5, ny5, nz5, x6, y6, z6, nx6, ny6, nz6, track->DefaultRibbon15Brush);
      if (poly2) scenerySegments->addPrimitive(poly2);
      Polygon3D *poly3 =
        DrawRibbon3D(track, g, xpos, ypos, zpos, x4, y4, z4, nx4, ny4, nz4, x5, y5, z5, nx5, ny5, nz5, track->DefaultRibbon14Brush);
      if (poly3) scenerySegments->addPrimitive(poly3);
      Polygon3D *poly4 =
        DrawRibbon3D(track, g, xpos, ypos, zpos, x3, y3, z3, nx3, ny3, nz3, x4, y4, z4, nx4, ny4, nz4, track->DefaultRibbon13Brush);
      if (poly4) scenerySegments->addPrimitive(poly4);
      Polygon3D *poly5 =
        DrawRibbon3D(track, g, xpos, ypos, zpos, x2, y2, z2, nx2, ny2, nz2, x3, y3, z3, nx3, ny3, nz3, track->DefaultRibbon12Brush);
...
void TrackSection::setAnnotation(char *ann)
{
  if (ann == NULL) {
    ...
    annotation = _strdup(ann);
  }
}

char *
  TrackSection::getAnnotation()
{
  return annotation;
}
...
HTREEITEM
TrackSection::LoadTree(CTrackTree *mainTree, HTREEITEM HGP2Track, int index, BOOL trackOrPit)
{
  ...
  if (trackSectionCmds) {
    ...
    if (NumberOfCmds > 0) {
      subTree =
        mainTree->insertInfoNode(HGP2TrackSection, CString("Dummy"), -1);
    }
  }
    ...
    if (NumberOfCmds > 0) {
      for (int j = 0; j < NumberOfCmds; j++) {
        ...
        if (cmd->getCmdIcon() == IDB_OBJECT) {
          ...
          int textureid = cmd->getNthArg(4);
          int loc = cmd->getNthArg(2);
          trackStrBuffer.Format(
            "Scenery Definition (0x%x) Loc=%s Texture=%s(0x%x)",
            cmd->getCmd(),
            getTextureLocation(loc),
            getTextureDescription(textureid),
            textureid);
          cmdnode = mainTree->insertInfoNode(HGP2TrackSection, trackStrBuffer, TO_ID(getTextureIcon(textureid)));
